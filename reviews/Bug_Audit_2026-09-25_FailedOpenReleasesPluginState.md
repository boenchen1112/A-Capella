...
- **The non-atomic project save** (`ProjectPersistenceService.cs:43`), already in #15's Runners-up, is the most likely way a user ends up with a truncated project file that triggers this ticket. It is unchanged and still worth doing, and after this fix a truncated file no longer costs the *current* project anything.
- **After a failed Open at HEAD, a new Melodyne edit on the same layer can replay the old session's render.** Reasoned only, and moot once this is fixed. `PitchCorrectionCache` is static and keyed by `(layerId, SourceKey-araEdit{gen}, backend)` (`MixEngine.cs:200-204`). After the release, the fresh session's edit generation restarts at 0. Its first edit produces `…-araEdit1`, which can still be cached from the destroyed session if that session had only 1-2 edits (`PitchCorrectionCache.cs:23`). The new edit would then be inaudible until a second edit. The same key-reuse shape can occur after a successful Open or New that reuses the same source file on the same `LayerId`. Not traced further.
